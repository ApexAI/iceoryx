...
TEST_F(PortManager_test, DeleteInterfacePortfromMaximumNumberAndAddOneIsSuccessful)
{
    ...
    // delete one and add one should be possible now
    {
        ...
        m_portManager->deletePortsOfProcess(iox::cxx::convert<std::string, iox::RuntimeName_t>(newProcessName));

        auto interfacePort = m_portManager->acquireInterfacePortData(
            iox::capro::Interfaces::INTERNAL, iox::cxx::convert<std::string, iox::RuntimeName_t>(newProcessName));
...
TEST_F(PortManager_test, AcquiringMaximumNumberOfNodesWorks)
{
    ...
    acquireMaxNumberOfNodes(nodeName, runtimeName, [&](auto node, auto newNodeName, auto newProcessName) {
        auto convertedNodeName = iox::cxx::convert<NodeName_t, std::string>(node->m_nodeName);
        auto convertedRuntimeName = iox::cxx::convert<RuntimeName_t, std::string>(node->m_runtimeName);
...
TEST_F(PortManager_test, DeleteNodePortfromMaximumNumberandAddOneIsSuccessful)
{
    ...
    iox::RuntimeName_t newProcessName =
        iox::cxx::convert<std::string, RuntimeName_t>(runtimeName + iox::cxx::convert::toString(i));
    iox::NodeName_t newNodeName =
        iox::cxx::convert<std::string, RuntimeName_t>(nodeName + iox::cxx::convert::toString(i));
...
TEST_F(PortManager_test, UnblockRouDiShutdownMakesAllPublisherStopOffer)
{
    ...
    for (unsigned int i = 0; i < iox::MAX_PUBLISHERS; i++)
    {
        ...
        auto publisherRuntimeName = iox::cxx::convert<std::string, iox::RuntimeName_t>("pub_" + std::to_string(i));
