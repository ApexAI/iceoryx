...
TEST_F(PoshRuntime_test, MaxAppNameLength)
{
    ...
    auto& runtime = PoshRuntime::initRuntime(iox::cxx::convert<std::string, iox::RuntimeName_t>(maxValidName));
...
TEST_F(PoshRuntime_test, getMiddlewarePublisherPublisherlistOverflow)
{
    ...
    for (; i < (iox::MAX_PUBLISHERS - iox::NUMBER_OF_INTERNAL_PUBLISHERS); ++i)
    {
        auto publisherPort = m_runtime->getMiddlewarePublisher(iox::capro::ServiceDescription(
            iox::cxx::convert<std::string, iox::RuntimeName_t>(convert::toString(i)),
            iox::cxx::convert<std::string, iox::RuntimeName_t>(convert::toString(i + 1U)),
            iox::cxx::convert<std::string, iox::RuntimeName_t>(convert::toString(i + 2U))));
        ASSERT_NE(nullptr, publisherPort);
    }
