...
| [DesignPattern](#design-pattern) | Certain code patterns which are repeating themselves all over the code are abstracted and stored in here. At the moment we only have the creation pattern but maybe there will be more. |
| [errors](#error-handling) | The Error-Handler to in iceoryx for handling fault cases. |
| [log](#log) | The logger used by iceoryx. |
| [rp](#relocatable-pointer) | Relocatable pointer used for shared memory |
| [posix](#posix-wrapper) | Posix constructs like shared memory, threads or semaphores are not used directly in our code base. We abstracted them so that they are following the [RAII (Resource Acquisition Is Initialization)](https://en.wikipedia.org/wiki/Resource_acquisition_is_initialization) idiom and other good practices from the C++ community. |
| [units](#units) | |
...
|`DeadlineTimer`       |   |   | Counts down a specific time and lets the user check if a deadline was already reached |
...
|`function`            |   |   | Heap and exception free implementation of `std::function`. Capacity has to be provided as template parameter. |
...
|`TACO`                    | i | X | Thread Aware exChange Ownership (TACO). Solution if you would like to use `std::atomic` with data types larger than 64 bit. Wait free data synchronization mechanism between threads.|
