...
After creating the runtime, the the client port is created and attached to the Waitset.
...
```cpp
    while (!iox::posix::hasTerminationRequested())
    {
        ...
        // wait with the waitset on a response
...
First, a callback is created that shall be called when server receives a response.
In this case the calculation is here done too but more resource consuming tasks
should be maybe outsourced.
...
From the server port is the `take()` API used to receive the data and then the `loan()` and send `send()` calls
to transmit the response after calculating the sum of the two numbers.
...
Once the user wants to shutdown the server it finally needed to detach from the registered server event:
<!-- [geoffrey] [iceoryx_examples/request_response/server_cxx_listener.cpp][wait for termination] -->
```cpp
