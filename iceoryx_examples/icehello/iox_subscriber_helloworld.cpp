...
using Implementations = Types<PosixSharedMemory<ShmBumpAllocator>>;
...
TYPED_TEST(SharedMemory_test, CreationWorksWithAppropriateParameters)
{
    ...
    auto mem = SharedMemoryCreator<PosixSharedMemory>()
                   .memorySizeInBytes(sizeGreaterZero)
                   .permissions(all)
                   .create<ShmBumpAllocator>(validName);
...
//// correct allocation will probably be tested separately for every allocator implementation so that the following
/// tests / can be removed / BEGIN allocation tests (remove later)
